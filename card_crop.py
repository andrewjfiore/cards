...
def _detect_text_boxes(img, min_conf=0.25, max_dim=1800):
    """Return list of 4-point OCR boxes in original-image coordinates."""
...
def _refine_quad_with_ocr(img, quad, min_conf=0.25, max_dim=1800, text_margin_frac=0.03):
    ...
    boxes = _detect_text_boxes(img, min_conf=min_conf, max_dim=max_dim)
    if not boxes:
        return quad, False
...
def process_image(src, dst, args, debug_dir=None):
    ...
    return True, f"OK ({strategy}{ocr_tag})"
